...
public class GetFilePathAsync_Tests : Query_Tests
{
	...
	public async Task Attachment_Not_Found_Returns_None_With_AttachmentNotFoundMsg()
	{
		// Arrange
		var (db, w, _) = Setup();
		var empty = new List<Attachment>().AsEnumerable().Wrap().ToResult();
	...
	public async Task Multiple_Attachments_Found_Returns_None_With_MultipleAttachmentsFoundMsg()
	{
		// Arrange
		var (db, w, _) = Setup();
		var list = new[] { new Attachment(), new Attachment() }.AsEnumerable().Wrap().ToResult();
	...
	public async Task Returns_Attachment_File_Path()
	{
		...
		var (db, w, v) = Setup();
		var urlPath = Rnd.Str;
		var single = new[] { new Attachment { UrlPath = urlPath } }.AsEnumerable().Wrap().ToResult();
