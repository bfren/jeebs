// Jeebs Unit Tests
// Copyright (c) bfren - licensed under https://mit.bfren.dev/2013

namespace Jeebs.WordPress.ContentFilters.Blocks.YouTube_Tests;

public class GetVideoId_Tests
{
	/// <summary>
	/// Test URLs from https://stackoverflow.com/a/27728417/8199362.
	/// </summary>
	public static TheoryData<string, string> Returns_Correct_Id_Data() =>
		[
			("https://www.youtube-nocookie.com/embed/up_lNV-yoK4?rel=0", "up_lNV-yoK4"),
			("https://www.youtube.com/user/Scobleizer#p/u/1/1p3vcRhsYGo", "1p3vcRhsYGo"),
			("https://www.youtube.com/watch?v=cKZDdG9FTKY&feature=channel", "cKZDdG9FTKY"),
			("https://www.youtube.com/watch?v=yZ-K7nCVnBI&playnext_from=TL&videos=osPknwzXEas&feature=sub", "yZ-K7nCVnBI"),
			("https://www.youtube.com/ytscreeningroom?v=NRHVzbJVx8I", "NRHVzbJVx8I"),
			("https://www.youtube.com/user/SilkRoadTheatre#p/a/u/2/6dwqZw0j_jY", "6dwqZw0j_jY"),
			("https://youtu.be/6dwqZw0j_jY", "6dwqZw0j_jY"),
			("https://www.youtube.com/watch?v=6dwqZw0j_jY&feature=youtu.be", "6dwqZw0j_jY"),
			("https://youtu.be/afa-5HQHiAs", "afa-5HQHiAs"),
			("https://www.youtube.com/user/Scobleizer#p/u/1/1p3vcRhsYGo?rel=0", "1p3vcRhsYGo"),
			("https://www.youtube.com/watch?v=cKZDdG9FTKY&feature=channel", "cKZDdG9FTKY"),
			("https://www.youtube.com/watch?v=yZ-K7nCVnBI&playnext_from=TL&videos=osPknwzXEas&feature=sub", "yZ-K7nCVnBI"),
			("https://www.youtube.com/ytscreeningroom?v=NRHVzbJVx8I", "NRHVzbJVx8I"),
			("https://www.youtube.com/embed/nas1rJpm7wY?rel=0", "nas1rJpm7wY"),
			("https://www.youtube.com/watch?v=peFZbP64dsU", "peFZbP64dsU"),
			("https://youtube.com/v/dQw4w9WgXcQ?feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://youtube.com/vi/dQw4w9WgXcQ?feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://youtube.com/?v=dQw4w9WgXcQ&feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://www.youtube.com/watch?v=dQw4w9WgXcQ&feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://youtube.com/?vi=dQw4w9WgXcQ&feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://youtube.com/watch?v=dQw4w9WgXcQ&feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://youtube.com/watch?vi=dQw4w9WgXcQ&feature=youtube_gdata_player", "dQw4w9WgXcQ"),
			("https://youtu.be/dQw4w9WgXcQ?feature=youtube_gdata_player", "dQw4w9WgXcQ")
		];

	[Theory]
	[MemberData(nameof(Returns_Correct_Id_Data))]
	public void Returns_Correct_Id(string input, string expected)
	{
		// Arrange
		var uri = new Uri(input);

		// Act
		var result = YouTube.GetVideoId(uri);

		// Assert
		Assert.Equal(expected, result);
	}

	[Fact]
	public void Incorrect_Url_Returns_Null()
	{
		// Arrange
		var uri = new Uri($"https://{Rnd.Str}.com");

		// Act
		var result = YouTube.GetVideoId(uri);

		// Assert
		Assert.Null(result);
	}
}
