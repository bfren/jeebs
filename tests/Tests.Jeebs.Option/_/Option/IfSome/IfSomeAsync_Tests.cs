...
public class IfSomeAsync_Tests : Jeebs_Tests.IfSomeAsync_Tests
{
	...
	{
		await Test00((opt, ifSome) => opt.IfSomeAsync(ifSome)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test01_None_Returns_Original_Option()
	{
		await Test01((opt, ifSome) => opt.IfSomeAsync(ifSome)).ConfigureAwait(false);
	}

	[Fact]
	public override async Task Test02_Some_Runs_IfSome_Func_And_Returns_Original_Option()
	{
		await Test02((opt, ifSome) => opt.IfSomeAsync(ifSome)).ConfigureAwait(false);
	}
}
