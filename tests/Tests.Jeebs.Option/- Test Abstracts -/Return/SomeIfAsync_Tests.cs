...
public abstract class SomeIfAsync_Tests
{
	...
	protected static async Task Test03(Func<Func<bool>, Task<int>, Handler, Task<Option<int>>> act)
	{
		...
		// Act
		var result = await act(() => true, Task.FromResult(value), DefaultHandler).ConfigureAwait(false);
	...
	protected static async Task Test05(Func<Func<bool>, Task<int>, Handler, Task<Option<int>>> act)
	{
		...
		// Act
		var result = await act(() => false, Task.FromResult(value), DefaultHandler).ConfigureAwait(false);
