...
namespace Jeebs
{
	...
	public static class Chain
	{
		...
		/// </summary>
		public static Task<IOk<bool>> CreateAsync()
		...
		/// <typeparam name="TState">Chain state type</typeparam>
		/// <param name="state">Chain state</param>
		public static Task<IOk<bool, TState>> CreateAsync<TState>(TState state)
			=> Task.FromResult(Create(state));
		...
		/// <typeparam name="TValue">Result value type</typeparam>
		/// <param name="value">Result value</param>
		public static Task<IOkV<TValue>> CreateVAsync<TValue>(TValue value)
			=> Task.FromResult(CreateV(value));
		...
		/// <param name="value">Result value</param>
		/// <param name="state">Chain state</param>
		public static Task<IOkV<TValue, TState>> CreateVAsync<TValue, TState>(TValue value, TState state)
			=> Task.FromResult(CreateV(value, state));
	}
}