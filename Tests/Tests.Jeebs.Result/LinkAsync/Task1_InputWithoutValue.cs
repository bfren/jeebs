...
{
	public class Task1_InputWithoutValue : ILinkAsync_Task1_InputWithoutValue
	{
		[Fact]
		public async Task StartSync_Successful_Returns_Ok()
		{
			...
			var chain = R.Chain;
			var index = 0;
			async Task t(IOk<bool> _) => await Task.Run(() => index++);
		...
		public async Task StartSync_Unsuccessful_Returns_Error()
		{
			// Arrange
			var chain = R.Chain;
			static async Task t(IOk<bool> _) => await Task.Run(() => throw new Exception("Something went wrong."));
		...
		public async Task StartSync_Unsuccessful_Adds_AsyncException_Message()
		{
			...
			static async Task t(IOk<bool> _) => await Task.Run(() => throw new Exception(msg));
		...
		public async Task StartSync_Unsuccessful_Then_SkipsAhead()
		{
			...
			var chain = R.Chain;
			var index = 0;
			static async Task t0(IOk<bool> _) => await Task.Run(() => throw new Exception("Something went wrong."));
			async Task t1(IOk<bool> _) => await Task.Run(() => index++);
