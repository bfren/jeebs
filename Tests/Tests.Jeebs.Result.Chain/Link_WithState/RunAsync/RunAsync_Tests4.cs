...
namespace Jeebs.LinkTests.WithState.Async
{
	...
	{
		[Fact]
		public void IOk_ValueType_WithState_Input_When_IOk_Runs_Action()
		{
			// Arrange
			const int state = 7;
			var chain = Chain.Create(state);
			var sideEffect = 1;
			async Task f(IOk<bool, int> _) => sideEffect++;
		...
		public void IOk_ValueType_WithState_Input_When_IOk_Catches_Exception()
		{
			...
			static async Task f(IOk<bool, int> _) => throw new Exception(error);
		...
		public void IOk_ValueType_WithState_Input_When_IError_Returns_IError()
		{
			...
			static async Task f(IOk<bool, int> _) => throw new Exception();
