// Jeebs Rapid Application Development
// Copyright (c) bfren - licensed under https://mit.bfren.dev/2013

using System.Collections.Generic;

namespace Jeebs.Constants;

/// <summary>
/// Bible Books.
/// </summary>
public static class BibleBooks
{
	/// <summary>
	///	Genesis.
	/// </summary>
	public static string Genesis =>
		"Genesis";

	/// <summary>
	///	Exodus.
	/// </summary>
	public static string Exodus =>
		"Exodus";

	/// <summary>
	///	Leviticus.
	/// </summary>
	public static string Leviticus =>
		"Leviticus";

	/// <summary>
	///	Numbers.
	/// </summary>
	public static string Numbers =>
		"Numbers";

	/// <summary>
	///	Deuteronomy.
	/// </summary>
	public static string Deuteronomy =>
		"Deuteronomy";

	/// <summary>
	///	Joshua.
	/// </summary>
	public static string Joshua =>
		"Joshua";

	/// <summary>
	///	Judges.
	/// </summary>
	public static string Judges =>
		"Judges";

	/// <summary>
	///	Ruth.
	/// </summary>
	public static string Ruth =>
		"Ruth";

	/// <summary>
	///	1 Samuel.
	/// </summary>
	public static string FirstSamuel =>
		"1 Samuel";

	/// <summary>
	///	2 Samuel.
	/// </summary>
	public static string SecondSamuel =>
		"2 Samuel";

	/// <summary>
	///	1 Kings.
	/// </summary>
	public static string FirstKings =>
		"1 Kings";

	/// <summary>
	///	2 Kings.
	/// </summary>
	public static string SecondKings =>
		"2 Kings";

	/// <summary>
	///	1 Chronicles.
	/// </summary>
	public static string FirstChronicles =>
		"1 Chronicles";

	/// <summary>
	///	2 Chronicles.
	/// </summary>
	public static string SecondChronicles =>
		"2 Chronicles";

	/// <summary>
	///	Ezra.
	/// </summary>
	public static string Ezra =>
		"Ezra";

	/// <summary>
	///	Nehemiah.
	/// </summary>
	public static string Nehemiah =>
		"Nehemiah";

	/// <summary>
	///	Esther.
	/// </summary>
	public static string Esther =>
		"Esther";

	/// <summary>
	///	Job.
	/// </summary>
	public static string Job =>
		"Job";

	/// <summary>
	///	Psalms.
	/// </summary>
	public static string Psalms =>
		"Psalms";

	/// <summary>
	///	Proverbs.
	/// </summary>
	public static string Proverbs =>
		"Proverbs";

	/// <summary>
	///	Ecclesiastes.
	/// </summary>
	public static string Ecclesiastes =>
		"Ecclesiastes";

	/// <summary>
	///	Song of Songs.
	/// </summary>
	public static string SongOfSongs =>
		"Song of Songs";

	/// <summary>
	///	Isaiah.
	/// </summary>
	public static string Isaiah =>
		"Isaiah";

	/// <summary>
	///	Jeremiah.
	/// </summary>
	public static string Jeremiah =>
		"Jeremiah";

	/// <summary>
	///	Lamentations.
	/// </summary>
	public static string Lamentations =>
		"Lamentations";

	/// <summary>
	///	Ezekiel.
	/// </summary>
	public static string Ezekiel =>
		"Ezekiel";

	/// <summary>
	///	Daniel.
	/// </summary>
	public static string Daniel =>
		"Daniel";

	/// <summary>
	///	Hosea.
	/// </summary>
	public static string Hosea =>
		"Hosea";

	/// <summary>
	///	Joel.
	/// </summary>
	public static string Joel =>
		"Joel";

	/// <summary>
	///	Amos.
	/// </summary>
	public static string Amos =>
		"Amos";

	/// <summary>
	///	Obadiah.
	/// </summary>
	public static string Obadiah =>
		"Obadiah";

	/// <summary>
	///	Jonah.
	/// </summary>
	public static string Jonah =>
		"Jonah";

	/// <summary>
	///	Micah.
	/// </summary>
	public static string Micah =>
		"Micah";

	/// <summary>
	///	Nahum.
	/// </summary>
	public static string Nahum =>
		"Nahum";

	/// <summary>
	///	Habakkuk.
	/// </summary>
	public static string Habakkuk =>
		"Habakkuk";

	/// <summary>
	///	Zephaniah.
	/// </summary>
	public static string Zephaniah =>
		"Zephaniah";

	/// <summary>
	///	Haggai.
	/// </summary>
	public static string Haggai =>
		"Haggai";

	/// <summary>
	///	Zechariah.
	/// </summary>
	public static string Zechariah =>
		"Zechariah";

	/// <summary>
	///	Malachi.
	/// </summary>
	public static string Malachi =>
		"Malachi";

	/// <summary>
	///	Matthew.
	/// </summary>
	public static string Matthew =>
		"Matthew";

	/// <summary>
	///	Mark.
	/// </summary>
	public static string Mark =>
		"Mark";

	/// <summary>
	///	Luke.
	/// </summary>
	public static string Luke =>
		"Luke";

	/// <summary>
	///	John.
	/// </summary>
	public static string John =>
		"John";

	/// <summary>
	///	Acts.
	/// </summary>
	public static string Acts =>
		"Acts";

	/// <summary>
	///	Romans.
	/// </summary>
	public static string Romans =>
		"Romans";

	/// <summary>
	///	1 Corinthians.
	/// </summary>
	public static string FirstCorinthians =>
		"1 Corinthians";

	/// <summary>
	///	2 Corinthians.
	/// </summary>
	public static string SecondCorinthians =>
		"2 Corinthians";

	/// <summary>
	///	Galatians.
	/// </summary>
	public static string Galatians =>
		"Galatians";

	/// <summary>
	///	Ephesians.
	/// </summary>
	public static string Ephesians =>
		"Ephesians";

	/// <summary>
	///	Philippians.
	/// </summary>
	public static string Philippians =>
		"Philippians";

	/// <summary>
	///	Colossians.
	/// </summary>
	public static string Colossians =>
		"Colossians";

	/// <summary>
	///	1 Thessalonians.
	/// </summary>
	public static string FirstThessalonians =>
		"1 Thessalonians";

	/// <summary>
	///	2 Thessalonians.
	/// </summary>
	public static string SecondThessalonians =>
		"2 Thessalonians";

	/// <summary>
	///	1 Timothy.
	/// </summary>
	public static string FirstTimothy =>
		"1 Timothy";

	/// <summary>
	///	2 Timothy.
	/// </summary>
	public static string SecondTimothy =>
		"2 Timothy";

	/// <summary>
	///	Titus.
	/// </summary>
	public static string Titus =>
		"Titus";

	/// <summary>
	///	Philemon.
	/// </summary>
	public static string Philemon =>
		"Philemon";

	/// <summary>
	///	Hebrews.
	/// </summary>
	public static string Hebrews =>
		"Hebrews";

	/// <summary>
	///	James.
	/// </summary>
	public static string James =>
		"James";

	/// <summary>
	///	1 Peter.
	/// </summary>
	public static string FirstPeter =>
		"1 Peter";

	/// <summary>
	///	2 Peter.
	/// </summary>
	public static string SecondPeter =>
		"2 Peter";

	/// <summary>
	///	1 John.
	/// </summary>
	public static string FirstJohn =>
		"1 John";

	/// <summary>
	///	2 John.
	/// </summary>
	public static string SecondJohn =>
		"2 John";

	/// <summary>
	///	3 John.
	/// </summary>
	public static string ThirdJohn =>
		"3 John";

	/// <summary>
	///	Jude.
	/// </summary>
	public static string Jude =>
		"Jude";

	/// <summary>
	///	Revelation.
	/// </summary>
	public static string Revelation =>
		"Revelation";

	/// <summary>
	/// Books of the Law.
	/// </summary>
	public static List<string> Law =>
		new([Genesis, Exodus, Leviticus, Numbers, Deuteronomy]);

	/// <summary>
	/// History Books.
	/// </summary>
	public static List<string> History =>
		new([Joshua, Judges, Ruth, FirstSamuel, SecondSamuel, FirstKings, SecondKings, FirstChronicles, SecondChronicles, Ezra, Nehemiah, Esther]);

	/// <summary>
	/// Wisdom Literature
	/// </summary>
	public static List<string> Wisdom =>
		new([Job, Psalms, Proverbs, Ecclesiastes, SongOfSongs]);

	/// <summary>
	/// Major Prophets.
	/// </summary>
	public static List<string> MajorProphets =>
		new([Isaiah, Jeremiah, Lamentations, Ezekiel, Daniel]);

	/// <summary>
	/// Minor Prophets.
	/// </summary>
	public static List<string> MinorProphets =>
		new([Hosea, Joel, Amos, Obadiah, Jonah, Micah, Nahum, Habakkuk, Zephaniah, Haggai, Zechariah, Malachi]);

	/// <summary>
	/// Old Testament.
	/// </summary>
	public static List<string> OldTestament =>
		[.. Law, .. History, .. Wisdom, .. MajorProphets, .. MinorProphets];

	/// <summary>
	/// Gospels.
	/// </summary>
	public static List<string> Gospels =>
		new([Matthew, Mark, Luke, John]);

	/// <summary>
	/// Gospels and Acts.
	/// </summary>
	public static List<string> GospelsAndActs =>
		new([.. Gospels, Acts]);

	/// <summary>
	/// Pauline Letters.
	/// </summary>
	public static List<string> PaulineLetters =>
		new([Romans, FirstCorinthians, SecondCorinthians, Galatians, Ephesians, Philippians, Colossians, FirstThessalonians, SecondThessalonians, FirstTimothy, SecondTimothy, Titus, Philemon]);

	/// <summary>
	/// Epistles.
	/// </summary>
	public static List<string> Epistles =>
		new([.. PaulineLetters, Hebrews, James, FirstPeter, SecondPeter, FirstJohn, SecondJohn, ThirdJohn, Jude, Revelation]);

	/// <summary>
	/// New Testament.
	/// </summary>
	public static List<string> NewTestament =>
		[.. GospelsAndActs, .. Epistles];

	/// <summary>
	/// All Books.
	/// </summary>
	public static List<string> All =>
		[.. OldTestament, .. NewTestament];
}
